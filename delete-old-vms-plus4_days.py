...
Number_of_days_to_eval_for_delete = 4
...
dict_of_vms_failed_to_delete = {}
dict_of_vms_we_deleted = {}
...
vcenter_server = 'xxxxxxxxxxxxxxxxxxxxxxxx'
...
#                print(str(date1combinedvmname2_split[1]) + " month is older then the current month "+str(time_string_current_split[1]))
            ...
            if int(date1combinedvmname2_split[2]) <= (int(time_string_current_split[2]) - Number_of_days_to_eval_for_delete):
...
def delete_vm(dict_of_vms_to_delete):
    for vmname_to_deletefunct in dict_of_vms_to_delete.keys():
        ...
        if vm_response.status_code == 204:
            ...
            dict_of_vms_we_deleted[vmid_to_delete_now] = vmname_to_deletefunct
        else:
            print(f"Failed to delete VM {vmname_to_deletefunct} Status code: {vm_response.status_code}")
            print(f"real error vm_response {vm_response.text}")
            dict_of_vms_failed_to_delete[vmid_to_delete_now] = vmname_to_deletefunct
            sys.exit()
...
if __name__ == "__main__":
    ...
    print("here are the vms we could not delete for some reason check the logs " + str(dict_of_vms_failed_to_delete))
    print("here are the vms we deleted " + str(dict_of_vms_we_deleted))
